...
import 'external/HearthisAtWidget.dart';
import 'external/IssuuWidget.dart';
import 'external/JWPlayerWidget.dart';
...
class WPContent extends StatelessWidget {
  ...
  Widget _buildParagraphs(BuildContext context, String rawContent) {
    ...
    if (paragraphs.isEmpty || paragraphs.length < 1) {
      return Container();
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: paragraphs.map((paragraph) => _buildParagraph(context, paragraph)).toList(),
      ),
    );
  }
  ...
  Widget _buildParagraph(BuildContext context, Paragraph paragraph) {
      ...
      return Container(
        ...
        child: RichText(
          text: TextSpan(
            ...
            style: DefaultTextStyle.of(context).style.copyWith(color: Theme.of(context).textTheme.headline6!.color, fontFamily: paragraph.fontFamily, fontSize: fontSize + 5.0),
          ),
    ...
    /* paragraph - text */
      ...
      return Container(
        ...
        child: RichText(
          text: TextSpan(
            children: paragraph.textSpans,
            style: DefaultTextStyle.of(context).style.copyWith(color: Theme.of(context).textTheme.subtitle2!.color, fontFamily: paragraph.fontFamily, fontSize: fontSize),
          ),
          textDirection: textDirection,
          textAlign: paragraph.textAlign!,
        ),
    ...
    /* paragraph - quote */
      ...
      return Container(
        ...
        child: RichText(
          text: TextSpan(
            children: paragraph.textSpans,
            style: DefaultTextStyle.of(context).style.copyWith(color: quoteTextColour, fontFamily: paragraph.fontFamily, fontSize: fontSize, fontWeight: FontWeight.bold),
          ),
          textDirection: textDirection,
          textAlign: paragraph.textAlign!,
        ),
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            /*RichText(
              text: TextSpan(
                children: paragraph.textSpans,
                style: DefaultTextStyle.of(context).style.copyWith(color: imageCaptionTextColor, fontFamily: fontFamily, fontSize: 0.7 * fontSize),
              ),
            )*/
          ],
        ),
    ...
    /* paragraph - youtube embed (https://youtube.com) */
      ...
      return youtubeEmbedWidget!.buildWithVideoId(context, paragraph.youtubeVideoId);
    }

    /* paragraph - soundcloud embed (https://soundcloud.com) */
    else if (paragraph.type == "soundcloud" && soundcloudEmbedWidget != null) {
      return soundcloudEmbedWidget!.buildWithTrackId(context, paragraph.soundcloudTrackId, paragraph.soudcloudEmbedCode);
    }

    /* paragraph - hearthis.at embed (https://hearthis.at) */
    else if (paragraph.type == "hearthis.at" && hearthisAtWidget != null) {
      return hearthisAtWidget!.buildWithTrackId(context, paragraph.hearthisAtTrackId);
    }
      ...
      contentParts.forEach((c) {
        if (c.startsWith("image")) {
          ...
          if (c.contains("<figcaption>")) {
            caption = c.substring(c.indexOf("<figcaption>") + 12, c.indexOf("</figcaption>"));
          }

          processedParagraphs.add(Paragraph.image(RegExp(r'src\s*=\s*"(.+?)"').firstMatch(c)!.group(1), caption, parseFigureCaptionHTML(caption, baseFontSize: 0.7 * fontSize)));
        } else if (c.startsWith("core-embed/issuu")) {
          processedParagraphs.add(Paragraph.issuu(SimpleArticle.pdfArticle(c, null)));
          ...
          try {
            switch (regExMatch.group(1)!.trim()) {
              case "jwplayer":
                processedParagraphs.add(Paragraph.jwplayer(regExMatch.group(2)));
                break;
              default:
                break;
            }
          ...
          processedParagraphs.add(Paragraph.youtubeEmbed(YouTubeWidget.getIdFromUrl(c)));
        } else if (c.startsWith("html")) {
          /* soundcloud - embed */
          if (c.contains("soundcloud.com")) {
            var soundcloudTrackId = SoundCloudWidget.getIdFromUrl(c);
            var soundcloudEmbedCode = c.substring(c.indexOf("-->") + 3, c.lastIndexOf("<!-- /wp:html -->"));
            if (soundcloudTrackId != null) {
              processedParagraphs.add(Paragraph.soundcloudEmbed(soundcloudTrackId, soundcloudEmbedCode));
            }
          }
          ...
          String headingContent = c.substring(c.indexOf("-->") + 3, c.lastIndexOf("<!-- /wp:heading -->")).replaceAll('\n', ' ').replaceAll('\r', ' ');

          bool isArabic = paragraphArabicIdentifier != null && headingContent.contains(paragraphArabicIdentifier!);
          ...
          processedParagraphs.add(parseHeadingHTML(headingContent, isArabic: isArabic, textAlign: textAlign, fontFamily: fontFamily, arabicFontFamily: arabicFontFamily, baseFontSize: fontSize));
        } else if (c.startsWith("paragraph")) {
          String paragraphContent = c.substring(c.indexOf("-->") + 3, c.lastIndexOf("<!-- /wp:paragraph -->")).replaceAll('\n', ' ').replaceAll('\r', ' ');

          bool isArabic = paragraphArabicIdentifier != null && paragraphContent.contains(paragraphArabicIdentifier!);
          ...
          processedParagraphs.add(parseParagraphHTML(paragraphContent, isArabic: isArabic, textAlign: textAlign, fontFamily: fontFamily, arabicFontFamily: arabicFontFamily, baseFontSize: fontSize));
        } else if (c.startsWith("quote")) {
          String paragraphContent = c.substring(c.indexOf("-->") + 3, c.lastIndexOf("<!-- /wp:quote -->")).replaceAll('\n', ' ').replaceAll('\r', ' ');

          bool isArabic = paragraphArabicIdentifier != null && paragraphContent.contains(paragraphArabicIdentifier!);

          processedParagraphs.add(parseQuoteHTML(paragraphContent, isArabic: isArabic, textAlign: quoteTextAlignment, fontFamily: fontFamily, arabicFontFamily: arabicFontFamily, baseFontSize: fontSize));
        } else if (c.startsWith("list")) {
          String listContent = c.substring(c.indexOf("-->") + 3, c.lastIndexOf("<!-- /wp:list -->")).replaceAll('\n', ' ').replaceAll('\r', ' ').replaceAll("<ol>", "").replaceAll("</ol>", "").replaceAll("</li>", "");
          ...
          bool isArabic = paragraphArabicIdentifier != null && listContent.contains(paragraphArabicIdentifier!);

          listItems.forEach((li) {
            processedParagraphs.add(parseListHTML(li, isArabic: isArabic, textAlign: textAlign, fontFamily: fontFamily, arabicFontFamily: arabicFontFamily, baseFontSize: fontSize));
          });
        }
      });
    } catch (exception) {
...
class Thumbnail extends StatelessWidget {
  ...
  const Thumbnail(this.imageURL, this.color, this.radius, {this.aspectRatio = 1.7});
